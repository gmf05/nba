...
### choose variables to put in model
#vars = c("wins", "fgpct1", "fgpct2", "fgpct3")

### estimate model & predict test game outcomes
# logistic regression
nbaMod = glm(win ~ ., data=nbaTrain[,vars], family="binomial")
#nbaPredict = predict(nbaMod, newdata=nbaTrain[,vars], type="response")
...
# 
# # decision tree
# nbaMod = rpart(win ~ ., data=nbaTrain[,vars], method="class", minbucket=50)
# prp(nbaMod) # plot tree
# nbaPredict = predict(nbaMod, newdata=nbaTest[,vars], type="class")
# 
# # random forest
# nbaMod <- randomForest(win ~ ., data=nbaTrain[,vars], nodesize=25, ntree=200)
# nbaPredict = predict(nbaMod, newdata=nbaTest[,vars])
